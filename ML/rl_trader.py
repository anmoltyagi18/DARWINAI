...
# ---------------------------------------------------------------------------
# FinRL Integration Wrappers
# ---------------------------------------------------------------------------

def train_finrl_agent(df: pd.DataFrame, initial_capital: float = 1_000_000, model_name: str = "ppo", total_timesteps: int = 50_000) -> Any:
    """
    Wrapper function to train a reinforcement learning trading agent using the FinRL framework.
    Learns trading strategies from historical data and simulates portfolio management.
    """
    try:
        from finrl.meta.env_stock_trading.env_stocktrading import StockTradingEnv
        from finrl.agents.stablebaselines3.models import DRLAgent
    except ImportError:
        print("[FinRL Wrapper] FinRL is not installed. Please install it to use this feature.")
        return None

    finrl_df = df.copy()
    if "date" not in finrl_df.columns and "index" in finrl_df.columns:
        finrl_df = finrl_df.rename(columns={"index": "date"})
    if "tic" not in finrl_df.columns:
        finrl_df["tic"] = "ASSET"
    ...
    try:
        ...
        e_train_gym = StockTradingEnv(df=finrl_df, **env_kwargs)
        ...
        print(f"[FinRL Wrapper] Error during training: {e}")
        return None


def predict_trade_action(model: Any, environment: Any, deterministic: bool = True) -> dict:
    """
    Wrapper function to predict the next trade action using a FinRL trained model.
    """
    try:
        from finrl.agents.stablebaselines3.models import DRLAgent
        
        # In a real FinRL environment DRL_prediction takes care of test simulation
        # Here we do a single step prediction as requested.
        action = DRLAgent.DRL_prediction(model=model, environment=environment)
        
        return {
            "action": "TRADE",
            "finrl_actions": action,
            "confidence": 1.0,
            "strategy_source": "finrl",
            "info": "Action predicted successfully via FinRL wrapper"
        }
    except ImportError:
        return {"action": "HOLD", "confidence": 0.0, "error": "FinRL not installed"}
    except Exception as e:
        print(f"[FinRL Wrapper] Error predicting action: {e}")
